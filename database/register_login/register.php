<?php
    ...
    // checking for the presence of the username in the system

    $count_query = "SELECT * FROM admin WHERE status = 1 AND username = :username";
    ...
    if($count_statement->rowCount() == 0){
        ...
        //checking for the presence of the phone number in the system

        $phone_query = "SELECT * FROM admin WHERE status = 1 AND phone_number = :number_";
        $phone_statement = $con->prepare($phone_query);
        $phone_statement->execute(
            array(
                ":number_" => $removed_brackets,
            )
        );
        if($phone_statement->rowCount() == 0){
            ...
            echo "User with same phone number already exists";
        }
    }
    else{
